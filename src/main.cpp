...
#if defined(ECHO_BASE) || (ECHO_BASE_PDM)
...
#define NUM_LEDS 18
CRGB leds[NUM_LEDS];
CRGB led_table[NUM_LEDS / 2] = {
    CRGB::Purple,     CRGB::MediumPurple, CRGB::Blue,
    CRGB::Green,      CRGB::LimeGreen,    CRGB::Yellow,
    CRGB::DarkOrange, CRGB::OrangeRed,    CRGB::Red};
#endif
...
static constexpr const size_t record_samplerate =
    16000;  // M5StickCPlus2だと48KHzじゃないと動かなかったが、M5Unified0.1.12で修正されたのとM5AtomS2+PDMUnitで不具合が出たので戻した。。
static int16_t* rec_data;

// setupの最初の方の機種判別で書き換えている場合があります。そちらもチェックしてください。（マイクの性能が異なるため）
uint8_t lipsync_shift_level =
    11;  // リップシンクのデータをどのくらい小さくするか設定。口の開き方が変わります。
float lipsync_max =
    LIPSYNC_LEVEL_MAX;  // リップシンクの単位ここを増減すると口の開き方が変わります。
#endif
...
bool echo_base = false;  // Echo BASE 利用時は true となる
...
void lipsync() {
    ...
    switch (M5.getBoard()) {
        ...
        default:
            ...
            if (echo_base) {
                size_t bytes_read = 0;
#if defined(ECHO_BASE) || (ECHO_BASE_PDM)
                rc = (i2s_read(I2S_NUM_0, rec_data, WAVE_SIZE, &bytes_read,
                               echobase.getDuration(WAVE_SIZE)) == ESP_OK);
#endif
            } else {
                rc = M5.Mic.record(rec_data, WAVE_SIZE, record_samplerate);
            }
            if (rc) {
                fft.exec(rec_data);
                for (size_t bx = 5; bx <= 60; ++bx) {
    ...
    // lipsync_maxを調整するときはこの行をコメントアウトしてください。
...
void setup() {
    bool rc;
    auto cfg = M5.config();
#if defined(PDM_PORTA) || (PDM_I2S) || (PDM_GPIO5_6) || (PDM_GPIO5_2) || \
    (ECHO_BASE) || (ECHO_BASE_PDM)
...
#ifndef SDL_h_
    rec_data = (typeof(rec_data))heap_caps_malloc(WAVE_SIZE * sizeof(int16_t),
                                                  MALLOC_CAP_8BIT);
    ...
    switch (M5.getBoard()) {
        ...
        case m5::board_t::board_M5AtomS3R:
            ...
            echo_base = true;
            echobase.init(16000 /*Sample Rate*/, 38 /*I2C SDA*/, 39 /*I2C SCL*/,
                          7 /*I2S DIN*/, 6 /*I2S WS*/, 5 /*I2S DOUT*/,
                          8 /*I2S BCK*/, Wire);
            i2s_set_clk(
                I2S_NUM_0, 16000, I2S_BITS_PER_CHAN_16BIT,
                I2S_CHANNEL_MONO);  // EchoBaseのライブラリのチャンネル形式が異なるため変更
            echobase.setMicGain(
                ES8311_MIC_GAIN_6DB);  // Set microphone gain to 6dB.
            Wire.end();  // I2Cのピン(GPIO38)をFastLEDで利用するためI2Cを停止
#endif
#if defined(ECHO_BASE_PDM)
            Wire.begin(38, 39, 100000U);
            Wire.beginTransmission(0x18);
            rc = Wire.endTransmission();
            Wire.end();
            if (rc == 0) {  // Echo BASE Found
                M5.Log.printf("M5.Log Found Echo BASE\n");
                echo_base = true;
                echobase.init(16000 /*Sample Rate*/, 38 /*I2C SDA*/,
                              39 /*I2C SCL*/, 7 /*I2S DIN*/, 6 /*I2S WS*/,
                ...
                echo_base = false;
                mic_cfg.pin_ws = 1;  //  PDM MIC (Grove Port)
                mic_cfg.pin_data_in = 2;
                M5.Mic.config(mic_cfg);
                M5.Mic.begin();
                uint16_t count = 0;
                ...
                for (int i = 0; i < WAVE_SIZE; i++) {
                    M5.Log.printf("%x ", rec_data[i]);
                    if (rec_data[i] > 0xfffff000) {
                        count++;
                    }
                }
                M5.Log.printf("M5.Log count %d\n",count);
                if (count < WAVE_SIZE / 4) {
                    M5.Log.printf("M5.Log Found PDM MIC\n");
                } else {  // GroveポートにPDMマイクが未接続の場合はAtom MICと仮定して動作
                    M5.Log.printf("M5.Log PDM MIC is not found(assume Atom MIC)\n");
                    M5.Mic.end();
                    mic_cfg.pin_ws = 5;  // Atom MIC
                    mic_cfg.pin_data_in = 6;
                    M5.Mic.config(mic_cfg);
                    M5.Mic.begin();
                }
...
#ifndef ECHO_BASE_PDM
    if (echo_base == false) {
        M5.Mic.begin();
    }
#endif

    // M5.Log.printf("M5.Log Setup Avatar\n");
...
#ifdef USE_FASTLED
    M5.Log.printf("M5.Log Setup FastLED\n");
    digitalWrite(LED_GPIO,
                 LOW);  // 1つ目のLEDが正常に動かないことがあるため追加
    FastLED.addLeds<SK6812, LED_GPIO, GRB>(
        leds, NUM_LEDS);  // GRB ordering is typical
